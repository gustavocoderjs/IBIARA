...
export function Market({ data, send, busy }: { data: ViewState; send: Send; busy: boolean }) {
    ...
    function reviewDraft(draft: CustomerDraft) {
        ...
        setDescription(draft.description); setBudget(draft.budget); setMinutes(String(draft.maxMinutes));
        setZone(draft.zone); setExcluded(draft.excluded.join(', ')); setReviewOpen(true);
    }
        ...
        <div className="customer-journey">
            <div className="customer-journey-main">
                <CustomerConversation disabled={busy || resumable || !!phase || Boolean(activeMandate)} completedOrderAt={latestOrder?.at}
                    onReview={reviewDraft} onDraftChanged={invalidateReview} onManualReview={message => {
                        invalidateReview(); setDescription(message); setReviewOpen(true);
                    }} />
                ...
                {activeMandate && <div className="mandate-summary"><div><LockKeyhole size={18} /><span>Autorização em aberto<strong>{money(mandate.maxCents)}</strong></span></div><p>Até {time(mandate.expiresAt)}</p><div className="flex gap-2 flex-wrap">{!resumable && <Button size="sm" variant="outline" disabled={busy || !!phase} onClick={resumeAuthorizedSearch}>Retomar autorização</Button>}<Button size="sm" variant="outline" disabled={busy || !!phase} onClick={async () => { if (await send({ type: 'revoke', mandateId: mandate.id })) { journey.current = {}; setResumable(false); setReviewOpen(false); } }}>Revogar</Button></div></div>}
            </div>
