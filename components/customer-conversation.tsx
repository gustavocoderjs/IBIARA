...
export function CustomerConversation({ disabled, completedOrderAt, onReview, onDraftChanged, onManualReview }: ConversationProps) {
    ...
    async function request(reset = false) {
        if (!view || inFlight.current || disabled || (!reset && (!text.trim() || needsNewRequest))) return;
        ...
        try {
            ...
            if (alive.current) {
                setView({ ...result.result, mode: view.mode }); setText(''); retry.current = null;
    ...
    return <section className="panel customer-conversation" aria-labelledby="customer-chat-title">
        ...
        {needsNewRequest ? <div className="customer-next-order"><p>Seu último pedido foi registrado. Comece uma nova conversa para escolher outra refeição e definir novos limites.</p><Button type="button" disabled={disabled || !!pending} onClick={() => void request(true)}><RotateCcw size={16}/>{pending === 'reset' ? 'Abrindo nova conversa…' : 'Começar novo pedido'}</Button></div> : <form onSubmit={submit} className="customer-composer">
