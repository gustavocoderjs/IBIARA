...
import { BuyerExperience } from '@/components/buyer-experience';
import { MerchantEconomy } from '@/components/merchant-economy';
import type { ViewState, Send, Dish } from '@/lib/client/workspace-types';
const merchantNav = [{ id: 'quick', label: 'Ajustes rápidos', icon: SlidersHorizontal }, { id: 'voice', label: 'Conversa por voz', icon: Mic }, { id: 'conversation', label: 'Conversa com a Byara', icon: MessageCircle }, { id: 'recipes', label: 'Fichas técnicas', icon: UtensilsCrossed }, { id: 'stock', label: 'Estoque e compras', icon: Package }, { id: 'policy', label: 'Limites do agente', icon: SlidersHorizontal }, { id: 'economy', label: 'Economia do agente', icon: Activity }, { id: 'orders', label: 'Pedidos', icon: ShoppingBag }];
const buyerNav = [{ id: 'experience', label: 'Conversar com meu agente', icon: MessageCircle }, { id: 'market', label: 'Meu próximo pedido', icon: Search }, { id: 'orders', label: 'Meus pedidos', icon: ShoppingBag }];
const titles: Record<string, [
    string,
    string
]> = { experience: ['Seu almoço começa com uma conversa.', 'Seu agente negocia. Você autoriza a proposta final.'], quick: ['Ajustes rápidos.', 'Pequenos ajustes para a cozinha seguir em frente.'], voice: ['Vamos conversar?', 'Fale, revise e ouça a Byara.'], conversation: ['Receita boa começa com conversa.', 'Conte o que você prepara. A Byara organiza os detalhes com você.'], recipes: ['De dar água na boca.', 'Fichas confirmadas são a base de cada proposta do seu agente.'], stock: ['Uma cozinha em dia.', 'Compras, disponibilidade e contagem no mesmo lugar.'], policy: ['A autonomia tem os seus limites.', 'Você define as regras. Seu agente negocia dentro delas.'], orders: ['Tem coisa boa saindo.', 'Sua operação, do pedido ao prato pronto.'], activity: ['Cada decisão deixa um registro.', 'Eventos da operação, filtrados para esta visão.'], integrations: ['Conexões da sua cozinha.', 'O que está funcionando e o que ainda precisa ser conectado.'], economy: ['A economia do seu agente.', 'Você define os limites. O agente representa sua cozinha no mercado sandbox.'], market: ['Hoje pede comida boa.', 'Seu agente encontra uma refeição dentro dos limites que você autorizar.'] };
...
export default function Workspace() {
 ...
 {view === 'economy' && r && <><MerchantEconomy data={data}/><section className='panel buyer-entry'><h2>Agora, experimente como cliente.</h2><p>Inicie uma busca no Exchange e autorize somente depois de conhecer a proposta.</p><Button disabled={busy} onClick={() => { changeRole('buyer'); setGuidedDemo(false); setView('experience'); }}>Experimentar como cliente <ArrowRight size={18}/></Button></section></>}
 ...
 {view === 'experience' && role === 'buyer' && <BuyerExperience send={send} busy={busy}/>}
