...
export default function Workspace() {
    const [role, setRole] = useState<Role>('buyer'), [view, setView] = useState('market'), [data, setData] = useState<ViewState | null>(null), [busy, setBusy] = useState(false), [error, setError] = useState('');
    const roleRef = useRef<Role>(role), inflight = useRef(false), initialView = useRef(false), lastContact = useRef(0);
    const [draft, setDraft] = useState('');
    const [customerRedirect, setCustomerRedirect] = useState<string | null>(null);
    ...
    const send: Send = async (command, options) => { if (inflight.current)
        ...
        if (!response.ok) {
            if (value.error?.code === 'CUSTOMER_FLOW_REQUIRED' && command.type === 'turn' && typeof command.text === 'string')
                setCustomerRedirect(command.text);
            if (response.status < 500 && value.error?.code !== 'CONCURRENT_UPDATE')
                replayRef.current = null;
            throw new Error(value.error?.message ?? 'Não foi possível salvar.');
        }
        replayRef.current = null;
        setCustomerRedirect(null);
 ...
 {role === 'merchant' && customerRedirect && <div className="panel p-4 mb-4" role="status"><p>Seu pedido de comida pode continuar com a Byara do consumidor. Só esta mensagem será levada; você poderá revisar e enviar.</p><Button className="mt-3" disabled={busy} onClick={() => { setCustomerPrefill(customerRedirect); changeRole('buyer'); }}>Continuar como consumidor<ArrowRight size={17}/></Button></div>}
 ...
 {view === 'market' && role === 'buyer' && <Market data={data} send={send} busy={busy} prefill={customerPrefill} onPrefillConsumed={() => setCustomerPrefill('')}/>}
 </>}
 <footer className="page-footer"><span><Sprout size={13}/>i.byara</span><span>{role === 'merchant' ? 'Sua cozinha. Seus limites. Agentes negociando.' : 'Sua intenção. Seus limites. Agentes negociando.'}</span><span>Demonstração · sem pagamentos reais</span></footer></main></div><MobileNavigation role={role} view={view} navigate={setView} pending={data?.orders.filter(o => o.status === 'CONFIRMED' || o.status === 'PREPARING').length ?? 0}/><Toaster richColors position="top-right"/></SidebarProvider>;
}
function Conversation({ data, send, busy, navigate, input, setInput }: {
    ...
    return <div className="conversation-grid"><section className="conversation-card panel"><div className="conversation-head"><div className="inline-flex items-center gap-3"><Mark /><div><h2>Byara</h2><p>Cadastro de receitas do restaurante</p></div></div><div className="chat-head-actions"><Tag tone="neutral">Interpretador local</Tag><Button onClick={() => navigate('voice')} variant="outline"><Mic size={18}/>Voz</Button></div></div><div className="chat-scroll" ref={messagesRef} role="log" aria-live="polite"><div className="chat-date">HOJE · SEU ESPAÇO DE CONVERSA</div>{r.conversation.map(m => <div className={`message ${m.role}`} key={m.id}>{m.role === 'assistant' && <Mark small/>}<div className="message-content"><div className="message-author">{m.role === 'assistant' ? 'Byara' : 'Você'}<time>{time(m.at)}</time></div><div className="message-bubble">{m.text}</div>{m.role === 'user' && <span className="message-saved"><CheckCheck size={12}/>Salvo</span>}</div></div>)}{busy && <div className="thinking"><Mark small/><span>Organizando sua cozinha<Loader2 className="spin" size={14}/></span></div>}</div><form className="composer" onSubmit={submit}><Textarea aria-label="Mensagem para a Byara" placeholder="Descreva a receita, os ingredientes e as quantidades…" disabled={busy} maxLength={4000} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault();
        void submit();
...
function Integrations() { return <><div className="integration-banner"><ShieldCheck size={22}/><div><h2>Uma demonstração transparente.</h2><p>Cálculos, negociação, persistência e reservas executam código real. O modo de IA depende da configuração do servidor; cada resposta identifica uso real ou simulado.</p></div></div><div className="integration-grid">{[{ title: 'NeuraLake', subtitle: 'Interpretação e memória', icon: Sprout, tag: 'Conexão configurável', tone: 'amber', text: 'Comprador e três restaurantes têm contextos separados. O cadastro da cozinha ainda usa regras locais. Cross Memory permanece desativada.', todo: 'Validar chamadas reais com as credenciais configuradas antes da apresentação. Falhas não ativam simulação automaticamente.' }, { title: 'Agora', subtitle: 'Conversa por voz', icon: Mic, tag: 'Mock · não conectado', tone: 'amber', text: 'Ditado e leitura do navegador podem ser usados, quando disponíveis. Nenhum áudio é enviado ao Agora.', todo: 'Próximo: contrato de voz, tokens por sessão, transcrição em português e reconexão.' }, { title: 'Documentos fiscais', subtitle: 'Custos e compras', icon: Receipt, tag: 'XML + exemplo', tone: 'green', text: 'Importação de XML NF-e/NFC-e com itens reconhecidos, deduplicação e confirmação separada de recebimento.', todo: 'Pendente: consulta por QR, novos emissores e conversões de embalagens não cadastradas.' }, { title: 'Comércio em sandbox', subtitle: 'Propostas, reservas e pedidos', icon: ShoppingBag, tag: 'Operacional', tone: 'green', text: 'Três agentes de restaurante com políticas próprias, ranking por total, reservas atômicas e pedidos idempotentes.', todo: 'Pagamento e entrega são simulados. Integrações comerciais dependem de homologação.' }].map(x => <section className="panel integration-card" key={x.title}><div className="section-heading"><span className="icon-tile"><x.icon size={24}/></span><Tag tone={x.tone}>{x.tag}</Tag></div><h2>{x.title}</h2><span className="integration-subtitle">{x.subtitle}</span><p>{x.text}</p><div className="integration-todo">{x.todo}</div></section>)}</div><div className="panel telemetry-card"><h3>Métricas de inferência</h3><div><span>Chamadas de LLM<strong>Consultar eventos</strong></span><span>Tokens<strong>Indisponível</strong></span><span>Custo de inferência<strong>Indisponível</strong></span><span>Cross Memory<strong>Desativada</strong></span></div></div></>; }
