...
import { MealScene, MotionSurface } from '@/components/motion-surface';
...
function Conversation({ data, send, busy, navigate, input, setInput }: {
    ...
    useEffect(() => { messagesRef.current?.scrollTo({ top: messagesRef.current.scrollHeight, behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' }); }, [r.conversation.length]);
